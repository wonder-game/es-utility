...
class FdManager
{
    ...
    {
        $rowKey = $this->getRowKey(static::UID_FD_KEY, $uid);
    ...
    {
        $rowKey = $this->getRowKey(static::FD_UID_KEY, $fd);
        $this->getTable(static::FD_UID_KEY)->set($rowKey, ['uid' => $uid, 'token' => $token]);
    }
    ...
    {
        $fdTable = $this->getTable(static::FD_UID_KEY);
        return $fdTable->exist($this->getRowKey(static::FD_UID_KEY, $fd));
    }
